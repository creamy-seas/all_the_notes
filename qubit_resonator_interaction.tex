...
\noindent  The  eigenstate spectrum  of  the  \iket{\pm,N} states  is
...
entangled states are
 ...
 \begin{itemize}
 \item  \blue{\textbf{Stark/Lambda   shift}:  photon-number-dependent
     shift        of        the        atom's        energy        by
     $\frac{g^{2}}{\Delta}\left( a^{\dagger}a + \frac{1}{2} \right)$};
 \item  \green{\textbf{Dispersive  shift:}  The  atom  ``pulls''  the
     cavity frequency by  $ \pm g^2/\Delta $ depending  on the atomic
     state} (we can neglect the constant offset as it will not affect
   transition energies.
 \end{itemize}
...
\begin{figure}[h]
  ...
  \caption{\small    When    there     is    0    detuning    (right,
    ...
    \green{$\pm\frac{g^{2}}{\Delta}$}     depending     on    state     of
...
\begin{figure}
  \centering \includegraphics[height=6cm]{qrLevel}
...
\item We apply a \textbf{fixed} resonator drive at $ \hbar\omega_r $.
  ...
  \blue{blue} terms in the above equations.
\item At  the degeneracy  point for  the new  qubit-resonator system,
  ...
  degeneracy  point.  \red{This  happens when  qubit is  biased to  a
    value were  $ \Delta E  =\hbar\omega_r$ i.e.  the level  separation is
    exactly equal  to the  drive from the  resonator.  For  all other
    cases there is no splitting/mixing.}
\end{itemize}
...
\noindent  and  without  the   \red{interaction  term}  one  had  the
...
\noindent  \red{\large \textbf{Solving  for  the Harmonic  oscillator
    alone}}, with a relaxation rate $ \kappa $ between the levels, it
...
\item Solving  the Master equation  for the continuous drive  for the
  ...
  can  be truncated  to have  only the  \iket{0} and  \iket{1} photon
  states.
...
\begin{framed}\noindent
  ...
  \item  Say   the  resonator  has  a   resonance  $  n  =   1  $  of
    \iunit{2}{GHz}.  When measuring the  qubit, each of the resonance
...
\red{\textbf{then irrelevant of what  the generator sweeps, the qubit
    will always  be giving  a response  so you  get a  vertical white
    line.}}
...
\item The  generator signal \red{2nd  tone} should be weak  enough to
