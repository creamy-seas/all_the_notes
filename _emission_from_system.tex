...
\begin{figure}[h]
  \centering \includegraphics[height=3cm]{noise}
\end{figure}

\noindent

\item The quantum  noise \textbf{\red{which is the  variation of current
...
\begin{figure}[h]
  \centering \includegraphics[height=4cm]{noise_spectrum}
\end{figure}
...
\end{itemize}
 ...
 Here it is  written, that relaxation occur due to  quantum noise in the
 system:
 \begin{equation}\label{key}
...
\noindent and \iaverage{\sigma_{21}} is  found from the stationary state
of the Master equation $ \dot{\rho}  = 0 $. The transmission coefficient
is:
...
\begin{itemize}
\item $ V_{\text{L}}^{+} $ resonant input with the \iket{i}\ilra\iket{j}
...
between the onset and emitted waves occur.
  ...
  Eq.~\eqref{rawTransformedFinal}.  The procedure  of solving the Master
  equation,    and    determining~$    \rho    $,    was    done    with
  \texttt{Mathematica}. The  $ \rho_{21}  $, $ \rho_{31}  $ coefficients
  obtained, for respective \iket{1}\ilra\iket{2}, \iket{1}\ilra\iket{3},
  drives, upon substitution into Eq.~\eqref{theoCoeff} give
...
ground state, and emits a photons that is coherent~\footnote{Of the same
  frequency.}   with the  incident  field,  but shifted  by  a phase  of
$ \pi $.\footnote{Signified by the $ i $ factor in Eq.~\eqref{theoCoeff}.}
Destructive interference occurs with the  incident field and the wave is
fully reflected.
...
\begin{figure}[h]
  ...
  \caption{\small  \textbf{Simulations  of   elastic  scattering  of  an
    ...
    with the atomic transition, $ \delta\omega_{ij}=0 $, the reflectance
    curve exhibits a peak, as  emission from the artificial atom undergo
    destructive  interference with  the incident  wave.  The  weaker the
...
grows, the photon emitted by the atom cannot interfere with all the ones
propagating  down the  transmission line.   This photon  overload causes
$  t $  and $  r  $ to  become  insensitive to  atomic transitions.   To
...
\item  Finding the  total emitted  power, by  integrating over  the full
  ...
  result in our rotation of the  state from \iket{0} to \iket{1} to form
  an intermediate value when $ \isigmaz = 0 $ and so the maximum emitted
  power from the atom will be:
...
\item  Now,  redoing  the  same,   but  only  considering  the  coherent
  ...
  \begin{itemize}
  \item \red{You can  measure this power with the SPA  i.e.  you measure
      1/8 of a single photon power;}
...
\item \textbf{Coherent field}, means a fixed phase relation with the two
  ...
  $ e^{i\omega_2t} \ira e^{i(\omega_1+\omega_2)t + \phi}$ where $ \phi $
  is a fixed value.  This allow further entanglement procedures;
\item \textbf{Emission  by qubit} can  be characterised as  coherent and
  ...
  \begin{figure}[h]
    \centering \includegraphics[height=7cm]{sphere}
  \end{figure}
  ...
  ``arrrow'' projects fully onto the equator,  it means that the atom is
  in            a            superposed            state            e.g.
  ...
  \red{Both  emissions  will  occur  at   the  frequency  of  the  qubit
    ...
    \isigmax will be sharp, while incoherent emission from \isigmaz will
    be broad as shown below}

\begin{figure}[h]
  \centering \includegraphics[height=5cm]{emission}
\end{figure}
