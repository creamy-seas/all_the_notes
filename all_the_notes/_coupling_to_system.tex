...
\section{Dipole operator for coupling \cite{Astafiev2010} \cite{abdumalikov2010} \label{sec:dipole_coupling}}
  ...
  \iframe{
    \begin{equation}\label{eq:couple1}
      \begin{aligned}
        ...
        \blue{\vartheta_{ij}                              =
          C_\text{q-mw}V_\text{qubit}\zeta_{ij}} &
      \end{aligned}
    \end{equation}
      
    \begin{itemize}
    \item $  \iabs{V_\text{mw}} $:\hfill  voltage amplitude
      in the transmission line;
    \item  $ C_{q-mw}  $:\hfill mutual  capacitance between
      the line and qubit;
    \item  $ V_\text{qubit}  $:\hfill persitent  voltage on
      the qubit;
    \item $ \zeta = \begin{pmatrix}
        \bra{0}\hat{A}\ket{0} & \bra{0}\hat{A}\ket{1} & \cdots\\
        \bra{1}\hat{A}\ket{0} & \bra{1}\hat{A}\ket{1} & \cdots\\
        \vdots & \vdots & \ddots
      \end{pmatrix} $:\hfill  normalised (to  unity) matrix
      elements.
    \end{itemize}
  }
...
\begin{minipage}{0.45\textwidth}
  ...
  \iframe{
    ...
    \begin{itemize}
    \item $ \iabs{I_{mw}} $:\hfill current amplitude in the
      transmission line;
    \item M:\hfill  mutual inductance between the  line and
      qubit;
...
\begin{minipage}{0.48\linewidth}
  \begin{enumerate}
  \item Writing out  the charging energy in  the qubit, due
    to  the   qubits,  $  Q_\text{qubit}  $,   and  induced
    capacitively charges:
  ...
  systems.\red{Therefore only the red one is carried on}.
...
\item \begin{equation}\label{eq:couple3}
    ...
    \end{aligned}
  \end{equation}
  \noindent  where we  have the  residual voltage  from the
  charge on the qubit $ V_\text{qubit} $.

\item Now let us take  the quantum mechanical operators and
  evaluate with the qubit states (which \textbf{do not} act
  on the microwave line operator $ \hat{V}_{mw} $):
...
\begin{minipage}{0.48\linewidth}
  ...
  \noindent  Only  the  red   terms  are  of  interest  for
  interaction,  since they  link  the  qubit and  microwave
  systems.\red{Therefore only the red one is left}.
...
\item Now let us take  the quantum mechanical operators and
  ...
  \iframe{
    ...
    \noindent \red{Effectively the  atom transitions from 1
      persistent current state to another. The flux that is
      induced on  the line  (through the  mutual inductance
      $ M $) is
		...
		Adding  a matrix  element  factor, so  that
...
\begin{minipage}{0.48\linewidth}
  \textbf{and recalling  that an  small flux change  with a
    current causes an energy change $ E = I\delta\phi $}
...
\item  $  \mathbf{\hat{V}_\text{qubit}}   $  is  the  qubit
  voltage operator, which next to a transition reads
	...
	\end{equation}
      ...
      \iframe{\begin{equation}
          \mathcal{H}_{\text{int}}=g_0\big(\sigma^++\sigma^-\big)\big(a+a^\dagger\big),
          \qquad g_0=V_{q0}C_{qr}V_{r0},
	\end{equation}}
      ...
      \item     Now,      to     express      the     value
        $  \hat{I}_{mw}  = \iabs{I_{mw}}\cos(\omega_dt)$  as  an
        operator, we follow these reasons:
        \begin{itemize}
        \item The current which supplied the energy couples
          the
        \end{itemize}
        \noindent to get
      ...
      \iframe{So  alltogether, the  operator  on the  qubit
        system is
...
\iframe{\noindent When  a qubit system emits,  the strength
  of emission depends  on the dipole moment  of the system,
  as we saw in Chapter~\ref{sec:dipole_coupling}:
...
\noindent   adding  some   time  dependence   of  $   \omega  $,
\textbf{since the system will repospond  at the rate of out
  driving}, and expanding out the matrix elements for a two
level system
...
\noindent we can concentrate on the \red{relaxation} matrix
element to get
