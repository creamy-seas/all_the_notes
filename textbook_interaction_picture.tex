...
\red{\textbf{\large Ultimately, what we have  found is that to solve for
    $H=H_0+V$, one needs to}} \textcolor{red}{\begin{enumerate}
...
\item $\hat{H}_\text{light}$ from before  (\red{find refference}) is the
...
\noindent where \red{$\hbar g(\vec{r})  = E_{0x}\sin(kz)\vec{d}$} is the
...
i.e. \red{\Large $\mathbf{\omega_a = \omega_j = \omega}$}
